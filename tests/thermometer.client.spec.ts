...
function waiting(overrides: Partial<ReasoningWaitProjection> = {}): ReasoningWaitProjection {
  return {
    ...
    active: true, tailKind: 'reasoning', ...overrides,
  }
}
...
describe('advanceReasoningWait', () => {
  ...
  it('clears immediately for reduced motion and identity changes', () => {
...
describe('extrapolateProjectionClock', () => {
  it('anchors to event elapsed and extrapolates with a monotonic frame clock', () => {
    const projection = waiting({ streamTime: ORIGIN + 3_000 })
    const first = extrapolateProjectionClock(projection, 50_000, null, IDENTITY)
    expect(first.elapsed).toBe(3_000)
    expect(extrapolateProjectionClock(projection, 55_000, first.anchor, IDENTITY).elapsed).toBe(8_000)
  })

  it('never moves backward when an event timestamp trails extrapolated time', () => {
    ...
    expect(delayed.elapsed).toBe(5_000)
  })

  it('resets for an inactive or different identity', () => {
    const first = extrapolateProjectionClock(waiting(), 10_000, null, IDENTITY)
    expect(extrapolateProjectionClock(waiting({ active: false }), 12_000, first.anchor, IDENTITY))
      .toEqual({ elapsed: 0, anchor: null })
    expect(extrapolateProjectionClock(waiting(), 12_000, first.anchor, 's2:1:1').elapsed).toBe(0)
  })
})
